...
{
	// Gutenprint moderno usa stp_dimension_t en estas funciones.
	// Aquí evitamos depender de stp_get_papersize_by_name(), que ya no está
	// disponible en algunos entornos actuales.
	stp_dimension_t pw = 0.0;
	stp_dimension_t ph = 0.0;
	stp_dimension_t mw = 0.0, mh = 0.0, nw = 0.0, nh = 0.0;
	stp_dimension_t l = 0.0, r = 0.0, t = 0.0, b = 0.0;
	...
	// Leemos el tamaño actual del medio configurado en Gutenprint.
	stp_get_media_size(stpvars, &pw, &ph);
	pagewidth = static_cast<int>(pw);
	pageheight = static_cast<int>(ph);

	// Leemos límites mínimos y máximos si el driver los expone.
	stp_get_size_limit(stpvars, &mw, &mh, &nw, &nh);
	minwidth = static_cast<int>(nw);
	maxwidth = static_cast<int>(mw);
	minheight = static_cast<int>(nh);
	maxheight = static_cast<int>(mh);

	// Si el driver no devolvió límites válidos, usamos el tamaño actual.
	if(minwidth <= 0)  minwidth = pagewidth;
	if(maxwidth <= 0)  maxwidth = pagewidth;
	if(minheight <= 0) minheight = pageheight;
	if(maxheight <= 0) maxheight = pageheight;

	stp_get_imageable_area(stpvars, &l, &r, &b, &t);

	Debug[TRACE] << "Imageable area from GP: L: " << l
	             << ", R: " << r
	             << ", T: " << t
	             << ", B: " << b << endl;
	...
	if(r>pagewidth)
	{
		rightbleed=static_cast<int>(r-pagewidth);
		pagewidth=static_cast<int>(r);
	}
	if(l<0)
	{
		pagewidth-=static_cast<int>(l);
		leftbleed=static_cast<int>(-l);
		r-=l;
		l=0;
	}
	if(b>pageheight)
	{
		bottombleed=static_cast<int>(b-pageheight);
		pageheight=static_cast<int>(b);
	}
	if(t<0)
	{
		pageheight-=static_cast<int>(t);
		topbleed=static_cast<int>(-t);
		b-=t;
		t=0;
	}

	leftmargin=static_cast<int>(l);
	topmargin=static_cast<int>(t);
	rightmargin=static_cast<int>(pagewidth-r);
	bottommargin=static_cast<int>(pageheight-b);
...
{
	MD5Consumer consumer;
	ImageSource_RainbowSweep *src=new ImageSource_RainbowSweep(256,256);

	SetProgress(p);
	Print(src,-1,-1,&consumer);

	delete src;

	return(consumer.GetPrintableDigest());
}